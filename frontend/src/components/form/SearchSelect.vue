...
<template>
  <div ref="rootEl" class="relative">
    ...
    <button
      type="button"
      class="h-10 w-full rounded-lg border bg-white text-left px-3 text-sm flex items-center justify-between gap-2"
      :class="disabled ? 'opacity-60 cursor-not-allowed' : 'hover:bg-gray-50'"
      :aria-expanded="open"
      @click="open ? closeMenu() : openMenu()"
      @keydown="onKeydown"
    >
      <span class="truncate" v-if="!open">
        <span v-if="selectedLabel" class="text-gray-900">{{ selectedLabel }}</span>
      ...
      <input
        ...
        v-model="query"
        type="text"
        class="w-full outline-none bg-transparent text-gray-900"
    ...
    <div
      v-if="open"
      class="absolute z-50 mt-1 w-full rounded-lg border bg-white shadow-lg max-h-60 overflow-auto"
      role="listbox"
    >
      <div
        ...
        :class="i === hoverIndex ? 'bg-gray-100' : ''"
        @mouseenter="hoverIndex = i"
        @mouseleave="hoverIndex = -1"
        @click="selectOption(opt)"
      >
        <span class="truncate">{{ opt[labelKey] ?? String(opt) }}</span>
        <span v-if="(opt[valueKey] ?? opt) === modelValue" class="text-xs text-gray-500">✓</span>
      </div>
      <div v-if="filtered.length === 0" class="px-3 py-2 text-sm text-gray-500">No results</div>
    </div>
  </div>
</template>